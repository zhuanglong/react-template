...
const devConfig = {
  ...
  plugins: [
    new ProgressBarPlugin({
      ...
      format: ` Avtion [:bar] ${chalk.green.bold(
        ":percent"
      )} (:elapsed seconds)`,
      clear: false,
      callback: () => {
        ...
        console.log(
          ` On Your Network:  ${chalk.green(
            `http://${devServer.ipAdress}:${devServer.port}/`
          )}`
        );
        console.log("\n\nNote that the development build is not optimized.");
        console.log(
          `To create a production build, use ${chalk.yellow("npm run build")}.`
        );
      }
